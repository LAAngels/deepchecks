...
try:
    from deepchecks_metrics import (
        f1_score,
        jaccard_score,
        precision_score,
        recall_score,
...
if TYPE_CHECKING:
    from deepchecks import (
        tabular,
    )  # pylint: disable=unused-import; it is used for type annotations
...
common_classification_metrics = {
    ...
    "precision_micro": make_scorer(
        precision_score, average="micro", zero_division=0, pos_label=None
    ),
    "precision_weighted": make_scorer(
        precision_score, average="weighted", zero_division=0, pos_label=None
    ),
    "recall_macro": make_scorer(
        recall_score, average="macro", zero_division=0, pos_label=None
    ),
    "recall_micro": make_scorer(
        recall_score, average="micro", zero_division=0, pos_label=None
    ),
    ...
    "jaccard_macro": make_scorer(
        jaccard_score, average="macro", zero_division=0, pos_label=None
    ),
    "jaccard_micro": make_scorer(
        jaccard_score, average="micro", zero_division=0, pos_label=None
    ),
    "jaccard_weighted": make_scorer(
        jaccard_score, average="weighted", zero_division=0, pos_label=None
    ),
}
...
multiclass_scorers_dict = {
    "roc_auc_per_class": make_scorer(
        roc_auc_per_class, response_method="predict_proba"
    ),
    ...
    "fpr_weighted": make_scorer(
        false_positive_rate_metric, averaging_method="weighted"
    ),
    ...
    "fnr_weighted": make_scorer(
        false_negative_rate_metric, averaging_method="weighted"
    ),
    ...
    "fpr_per_class": make_scorer(
        false_positive_rate_metric, averaging_method="per_class"
    ),
    "fnr_per_class": make_scorer(
        false_negative_rate_metric, averaging_method="per_class"
    ),
    "tnr_per_class": make_scorer(
        true_negative_rate_metric, averaging_method="per_class"
    ),
...
class DeepcheckScorer:
    ...
    def __init__(
        ...
        if isinstance(scorer, str):
            formatted_scorer_name = (
                scorer.lower()
                .replace("sensitivity", "recall")
                .replace("specificity", "tnr")
                .replace(" ", "_")
            )
            ...
            if formatted_scorer_name in _str_to_scorer_dict:
                ...
                try:
                    ...
                    raise errors.DeepchecksValueError(
                        f"Scorer name {scorer} is unknown. "
                        f"See metric guide for a list of allowed scorer names."
                    ) from e
        ...
        self._is_proba_scorer = (
            has_proba_scorer_import
            and isinstance(self.scorer, (_ProbaScorer, _ThresholdScorer))
        ) or (
            not has_proba_scorer_import
            and "predict_proba" in getattr(self.scorer, "_response_method", "")
        )
    ...
    def run_on_pred(self, y_true, y_pred=None, y_proba=None):
        ...
        if isinstance(self.scorer, _BaseScorer):
            if y_proba is not None and self._is_proba_scorer:
                ...
                pred_to_use = y_pred
            return (
                self.scorer._score_func(y_true, pred_to_use, **self.scorer._kwargs)
                * self.scorer._sign
            )
    ...
    def _wrap_classification_model(self, model, data_):
        ...
        class MyModelWrapper:
            ...
            def predict(self, data: pd.DataFrame) -> np.ndarray:
                ...
                if self.is_binary:
                    transfer_func = np.vectorize(
                        lambda x: 0 if x == self.model_classes[0] else 1
                    )
                ...
                # In case of multiclass with single label, convert into multi-label
                    ...
                    if (len(predictions) != 0) and is_sequence_not_str(
                        next(iter(predictions))
                    ):
                        predictions = np.array([np.array(x) for x in predictions])
                    predictions = _transform_to_multi_label_format(
                        predictions, self.model_classes
                    )
                return predictions
            ...
            def classes_(self):
                return np.asarray(
                    [0, 1] if len(self.model_classes) == 2 else self.model_classes
                )
    ...
    def _run_score(self, model, data: pd.DataFrame, label_col: pd.Series):
        ...
        if self._is_proba_scorer and not hasattr(
            model, "predict_proba"
        ):  # pylint: disable=protected-access
            raise errors.DeepchecksValueError(
        ...
        if self.model_classes is not None:
            ...
            if model.is_binary:
                if len(label_col.unique()) > 2:
                    raise errors.DeepchecksValueError(
                        "Model is binary but the label column has more than 2 classes: "
                        f"{label_col.unique()}"
                    )

                label_col = label_col.map(
                    {self.model_classes[0]: 0, self.model_classes[1]: 1}
                )
            else:
                ...
                label_col = _transform_to_multi_label_format(
        ...
        if self.model_classes is not None and isinstance(scores, np.ndarray):
            ...
            if (
                ...
                unseen_class = (
                    ...
                    if seen_class == self.model_classes[1]
    ...
    def validate_scorer_multilabel_output(self, scores):
        ...
        if self.model_classes is not None and isinstance(scores, t.Sized):
            ...
            scores.update(
                {
                    class_name: np.nan
                    for class_name in set(self.observed_classes)
    ...
    def __call__(self, model, dataset: "tabular.Dataset"):
        ...
        return self._run_score(
            model,
            dataset_without_nulls.features_columns,
            dataset_without_nulls.label_col,
        )
    ...
    def score_perfect(self, dataset: "tabular.Dataset"):
        ...
        score = self._run_score(
            perfect_model, dataset.features_columns, dataset.label_col
        )
        if isinstance(score, dict):
            ...
            if any(score_values != first_score):
                get_logger().warning(
                    "Scorer %s return different perfect score for different classes",
                    self.name,
                )
            return first_score
        return score
    ...
    def validate_fitting(self, model, dataset: "tabular.Dataset"):
        ...
        single_label_data = (
            dataset.data[dataset.label_col.notna()].groupby(dataset.label_name).head(1)
        )
        new_dataset = dataset.copy(single_label_data)
        result = self._run_score(
            model, new_dataset.features_columns, new_dataset.label_col
        )
        ...
        if isinstance(result, dict):
            ...
            raise errors.DeepchecksValueError(
                f"Expected scorer {self.name} to return number or dict "
...
def get_default_scorers(model_type, class_avg: bool = True):
    ...
    """
    return_array = (
        model_type in [TaskType.MULTICLASS, TaskType.BINARY] and class_avg is False
    )
...
def init_validate_scorers(
    ...
    if isinstance(scorers, t.Mapping):
        scorers = [
            DeepcheckScorer(scorer, model_classes, observed_classes, name)
            for name, scorer in scorers.items()
        ]
    else:
        scorers = [
            DeepcheckScorer(scorer, model_classes, observed_classes)
...
def _transform_to_multi_label_format(y: np.ndarray, classes):
    ...
    if y.ndim == 1:
        kwargs = (
            {"sparse_output": False}
            if version.parse(scikit_version) >= version.parse("1.2")
            else {"sparse": False}
        )
        ohe = OneHotEncoder(
    ...
    # If after squeeze there are still 2 dimensions, then it must have column for each model class.
        ...
        raise errors.DeepchecksValueError(
            f"got y with unworkable shape: {y.shape}. {SUPPORTED_MODELS_DOCLINK}"
        )
