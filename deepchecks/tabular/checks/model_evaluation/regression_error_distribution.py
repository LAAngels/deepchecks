...
class RegressionErrorDistribution(SingleDatasetCheck):
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        y_pred = pd.Series(
            y_pred, name="predicted " + str(dataset.label_name), index=y_test.index
        )
        ...
        if context.with_display:
            ...
            n_largest = pd.concat(
                [
                    dataset.data.loc[n_largest_diff.index],
                    y_pred.loc[n_largest_diff.index],
                    n_largest_diff,
                ],
                axis=1,
            )
            ...
            n_smallest = pd.concat(
                [
                    dataset.data.loc[n_smallest_diff.index],
                    y_pred.loc[n_smallest_diff.index],
                    n_smallest_diff,
                ],
                axis=1,
            )
            ...
            fig.add_vline(
                ...
                annotation_position=(
                    "top left" if np.median(diff) < np.mean(diff) else "top right"
                ),
            )
            fig.add_vline(
                x=np.mean(diff),
                ...
                annotation_position=(
                    "top right" if np.median(diff) < np.mean(diff) else "top left"
                ),
            )
    ...
    def add_condition_kurtosis_greater_than(self, threshold: float = -0.1):
        ...
        def min_kurtosis_condition(result: Dict[str, float]) -> ConditionResult:
            details = (
                f'Found kurtosis value of {format_number(result["Kurtosis Value"], 5)}'
            )
            category = (
                ConditionCategory.PASS
                if result["Kurtosis Value"] > threshold
                else ConditionCategory.WARN
            )
            return ConditionResult(category, details)

        return self.add_condition(
            f"Kurtosis value higher than {format_number(threshold, 5)}",
    ...
    def add_condition_systematic_error_ratio_to_rmse_less_than(
        ...
        return self.add_condition(
            f"Systematic error ratio lower than {format_number(max_ratio)}",
            max_bias_condition,
        )
